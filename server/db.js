...
// ── Schema bootstrap ──────────────────────────────────────────────────────────
// Checks once whether the schema exists, creates tables only if needed.
// One cheap round-trip on first call, then fully skipped.
...
async function bootstrap() {
  ...
  // Check if all tables exist in a single query
  const { rows } = await sql.query(
    `SELECT COUNT(*) AS n FROM information_schema.tables
     WHERE table_schema = 'public'
     AND table_name IN (
       'products','orders','customers','order_items',
       'admin_notifications','customer_messages','landing_images','announcements'
     )`,
    [],
    { fullResults: true },
  );
  ...
  // Mark done early so recursive calls (unlikely but safe) don't loop
  bootstrapped = true;

  if (found === 8) return; // all tables present — skip DDL entirely
...
// ── Query helpers ─────────────────────────────────────────────────────────────
// Each helper calls bootstrap() first — it's a no-op after the first call.
